...
async function callNoonUpdateOrder(
  cookie: string,
  payload: Record<string, unknown>
): Promise<{ ok: boolean; status: number; statusText: string; text: string; body: unknown }> {
  const response = await fetch(NOON_UPDATE_ORDER_URL, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Cookie: cookie,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const mpItemNr =
      typeof parsed.mp_item_nr === "string" ? parsed.mp_item_nr.trim() : ""

    if (!fbpiOrderNr || !mpItemNr) {
      return new Response(
        JSON.stringify({
          ok: false,
          error: "`fbpi_order_nr` and `mp_item_nr` are required.",
        }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      )
    }

    // 1. Call the Noon UpdateOrder API to mark the item as out of stock.
    const payload = {
    ...
    let result = await callNoonUpdateOrder(cookie, payload)

    if (result.status === 401) {
      const freshCookie = await getSessionCookie(true)
      result = await callNoonUpdateOrder(freshCookie, payload)
    }
    ...
    // 2. Update the local order_items table to reflect the new status.
    const { error: updateError } = await supabase
      .from("order_items")
      .update({ integration_status: "OUT_OF_STOCK" })
      .eq("mp_item_nr", mpItemNr)
    ...
    if (updateError) {
      return new Response(
        JSON.stringify({
          ok: false,
          error: `Noon accepted the request but the local update failed: ${updateError.message}`,
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    return new Response(
      JSON.stringify({
        ...
        data: {
          fbpi_order_nr: fbpiOrderNr,
          mp_item_nr: mpItemNr,
          integration_status: "OUT_OF_STOCK",
        },
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    )
